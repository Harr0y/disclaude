...
Claude Code Skills 是基于 **Agent Skills 开放标准** 的扩展机制，允许你自定义 Claude 的行为和能力。通过创建 `SKILL.md` 文件，你可以为 Claude 添加新的 slash 命令、注入领域知识、或自动化复杂工作流程。
...
| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `name` | string | ❌ | 技能名称，作为 `/slash-command` 使用。小写字母、数字、连字符，最多 64 字符 |
| `description` | string | ✅ | 技能功能描述。Claude 据此判断何时自动加载技能 |
| `argument-hint` | string | ❌ | 参数提示，如 `[issue-number]` 或 `[file] [format]` |
...
|------|-----------|---------------|---------------|
| 默认 | ✅ | ✅ | 描述常驻，完整内容在调用时加载 |
| `disable-model-invocation: true` | ✅ | ❌ | 仅在用户手动调用时加载 |
| `user-invocable: false` | ❌ | ✅ | 描述常驻，完整内容在调用时加载 |
...
# ❌ 错误格式
allowed-tools: ["Read", "Write", "Edit"]  # 非官方标准
...
description: This skill explains code in a very detailed manner with lots of examples and best practices and diagrams...  # 太冗长
```
...
**研究步骤**：
1. 使用 Glob 查找相关文件：`**/*$ARGUMENTS*`
2. 使用 Grep 搜索关键词：`$ARGUMENTS`
3. 阅读 Top 10 相关文件
4. 分析架构和实现模式
5. 总结发现并提供文件引用
...
**⚠️ 注意**：`context: fork` 仅适用于有明确任务的技能。如果技能只包含指南而无具体任务，子代理会收到指南但无法执行。
...
生成的 `codebase-map.html` 将在浏览器中自动打开。
```

**scripts/visualize.py**（Python 脚本）：
```python
#!/usr/bin/env python3
import json
...
if __name__ == '__main__':
    ...
    generate_html(data)
    print(f'✅ Generated codebase-map.html')
```
...
为目标文件生成 Markdown 格式的 API 文档。

**目标文件**：$ARGUMENTS

**步骤**：

1. 使用 Grep 搜索所有 API 端点：
   \```
...
**文档维护**：如发现错误或有改进建议，请提交 issue 或 PR。
