 ...
 * - Returns results to OrchestrationAgent for evaluation
 *
 ...
 * The orchestration agent handles all user communication.
 *
 * NEW Dialogue Flow:
 * - User request from Task.md → this agent FIRST
 * - This agent's output → OrchestrationAgent (evaluation)
 * - OrchestrationAgent's next instructions → this agent
 * - Loop continues until OrchestrationAgent signals completion
 */
...
export class ExecutionAgent {
  ...
  private logger = createLogger('ExecutionAgent', { model: '' });
  ...
  constructor(config: ExecutionAgentConfig) {
    ...
    this.logger = createLogger('ExecutionAgent', { model: this.model });
  }
  ...
  private async loadSkill(): Promise<void> {
    const result = await loadSkill('execution-agent');
    if (!result.success || !result.skill) {
      throw new Error(
        `ExecutionAgent skill is required but failed to load. ` +
        `Error: ${result.error || 'Unknown error'}. ` +
        `Please ensure .claude/skills/execution-agent/SKILL.md exists and is valid.`
      );
    }
    ...
    }, 'ExecutionAgent skill loaded');
  }

  /**
   * Create SDK options for execution agent.
  ...
  private createSdkOptions(resume?: string): Record<string, unknown> {
    ...
    const mcpServers = getSkillMcpServers('execution-agent') || {
      playwright: {
        type: 'stdio',
        command: 'npx',
  ...
  async *queryStream(prompt: string, sessionId?: string): AsyncIterable<AgentMessage> {
    ...
    this.logger.debug({ sessionId, promptLength: prompt.length }, 'Starting execution query');
    ...
    try {
      ...
      this.logger.error({ err: error }, 'Execution query failed');
      yield {
